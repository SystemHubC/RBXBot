...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in last_sent_time and (time.time() - last_sent_time[user_id]) < COOLDOWN_TIME:
        ...
        await update.message.reply_text(f'You have submitted your request too many times, please try again in {int(remaining_time)} seconds.')
    ...
    try:
        ...
        # Ожидание 5 секунд перед запросом ID плагина
        time.sleep(5)
